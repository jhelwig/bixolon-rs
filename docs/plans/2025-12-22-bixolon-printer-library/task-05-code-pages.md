...
**Files:**
- Create: `src/command/codepage.rs`
...
```bash
cargo test -p bixolon command::codepage
```
...
## Step 4: Implement code page commands

Update `src/command/codepage.rs`:
...
```bash
git add src/command/codepage.rs src/command/mod.rs
git commit -m "feat: add code page and international character set commands"
```
