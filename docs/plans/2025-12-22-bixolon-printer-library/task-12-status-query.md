# Task 12: Status and Query Commands

> **Context:** See [index.md](./index.md) for architecture and rationale

**Files:**
- Create: `src/command/status.rs`
- Modify: `src/command/mod.rs` (add module, add `QueryCommand` trait)
...
|---------|---------|-------------|
| `TransmitStatus` | `DLE EOT n` | Real-time status transmission |
| `TransmitPaperStatus` | `GS r n` | Transmit paper sensor status |
| `EnableAsb` | `GS a n` | Enable Automatic Status Back |

---

## Step 1: Add QueryCommand trait to mod.rs
...
pub trait QueryCommand: Command {
    ...
    /// Parse the response bytes from the printer.
...
## Step 2: Implement status commands
...
use super::{Command, QueryCommand, DLE, GS};
...
impl AsbFlags {
    ...
    fn to_byte(self) -> u8 {
        let mut n = 0u8;
        if self.drawer { n |= 0x01; }
        if self.online_offline { n |= 0x02; }
        if self.error { n |= 0x04; }
        if self.paper_roll { n |= 0x08; }
        n
    }
}
...
```bash
git add src/command/status.rs src/command/mod.rs
git commit -m "feat: add status query commands and ASB"
```
