# Task 9: Barcode Commands

> **Context:** See [index.md](./index.md) for architecture and rationale

**Files:**
- Create: `src/command/barcode.rs`
- Modify: `src/command/mod.rs` (add module)

**Important:** Barcode data validation uses miette for span-based error diagnostics.
...
|---------|---------|-------------|
| `SetBarcodeHeight` | `GS h n` | Set barcode height (1-255 dots) |
| `SetBarcodeWidth` | `GS w n` | Set barcode module width (2-6) |
| `SetHriPosition` | `GS H n` | Set HRI character position |
| `SetHriFont` | `GS f n` | Set HRI character font |
| `PrintBarcode` | `GS k m n d...` | Print barcode with data |
...
impl PrintBarcode {
    ...
    fn validate(system: BarcodeSystem, data: &[u8]) -> Result<(), BarcodeError> {
        ...
        if matches!(system, BarcodeSystem::Itf) && data.len() % 2 != 0 {
        ...
        for (i, &byte) in data.iter().enumerate() {
            let valid = match system {
                BarcodeSystem::UpcA | BarcodeSystem::UpcE |
                BarcodeSystem::Jan13 | BarcodeSystem::Jan8 |
                ...
                BarcodeSystem::Codabar => {
                    byte.is_ascii_digit() || (b'A'..=b'D').contains(&byte) ||
...
```bash
git add src/command/barcode.rs src/command/mod.rs
git commit -m "feat: add barcode commands with validation"
```
