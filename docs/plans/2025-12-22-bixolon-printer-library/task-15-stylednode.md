# Task 15: StyledNode AST

> **Context:** See [index.md](./index.md) for architecture and rationale

**Files:**
- Create: `src/style/text.rs`
- Modify: `src/style/mod.rs` (add exports)

**Important:** StyledNode is an AST that handles nested styles correctly. The `Styleable` trait allows ergonomic syntax like `"text".bold().underlined()`.

---

## Core Concepts

1. **StyledNode enum**: Either plain `Text` or `Styled` with children
2. **Styleable trait**: Enables fluent API on strings
3. **Rendering**: Walk AST with style stack, generate commands and text
...
use crate::command::character::UnderlineMode;
use crate::command::{Command, basic::LineFeed};
...
impl StyledNode {
    ...
    ///
    /// If both are styled nodes with the same parent, the children are merged.
    pub fn append(self, other: StyledNode) -> Self {
        match self {
            StyledNode::Styled { style, mut children } => {
                children.push(other);
        ...
        match self {
            ...
            StyledNode::Styled { style, children } => {
                ...
                *current_effective = new_effective;
                ...
                let transition_back = style_transition_commands(current_effective, &popped_effective);
...
pub trait Styleable: Sized {
    ...
    fn underlined(self) -> StyledNode {
        self.into_node().with_style(StyleSet::default().with_underline(UnderlineMode::Single))
    }

    /// Apply double underline style.
    fn double_underlined(self) -> StyledNode {
        self.into_node().with_style(StyleSet::default().with_underline(UnderlineMode::Double))
    }
...
mod tests {
    ...
    fn nested_styles_both_apply() {
        ...
        let node = "Hello".bold().with_style(StyleSet::default().underlined());
        let output = node.render();

        // Should have underline on, then bold on
        let underline_on_pos = output.windows(3)
            .position(|w| w == [ESC, b'-', 1]);
        let bold_on_pos = output.windows(3)
            .position(|w| w == [ESC, b'E', 1]);
    ...
    fn style_reset_after_pop() {
        ...
        let underline_style = StyleSet::default().underlined();
        ...
        let underline_on_count = bytes.windows(3)
            .filter(|w| *w == [ESC, b'-', 1])
            .count();
        let underline_off_count = bytes.windows(3)
            .filter(|w| *w == [ESC, b'-', 0])
            .count();
...
```bash
git add src/style/text.rs src/style/mod.rs
git commit -m "feat: add StyledNode AST and Styleable trait"
```
