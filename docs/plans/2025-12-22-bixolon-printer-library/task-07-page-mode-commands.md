# Task 7: Page Mode Commands

> **Context:** See [index.md](./index.md) for architecture and rationale

**Files:**
- Create: `src/command/page_mode.rs`
- Modify: `src/command/mod.rs` (add module)

**Important:** Only implement commands explicitly documented for page mode. Hardware verification required before adding additional convenience methods.
...
|---------|---------|-------------|
| `EnterPageMode` | `ESC L` | Switch to page mode |
| `ExitPageMode` | `ESC S` | Return to standard mode |
| `SetPrintDirection` | `ESC T n` | Set print direction in page mode |
| `SetPrintArea` | `ESC W ...` | Set printing area boundaries |
| `SetVerticalPosition` | `GS $ nL nH` | Set absolute vertical position in page mode |
...
## Step 2-3: Write tests and implement
...
impl PrintArea {
    ...
    pub const fn default_80mm() -> Self {
        Self { x: 0, y: 0, width: 512, height: 1662 }
    }

    /// Default print area for 58mm paper.
    pub const fn default_58mm() -> Self {
        Self { x: 0, y: 0, width: 360, height: 1662 }
    }
}
...
impl Command for SetPrintArea {
    fn encode(&self) -> Vec<u8> {
        ...
        vec![
            ESC, b'W',
            (area.x & 0xFF) as u8, ((area.x >> 8) & 0xFF) as u8,
            (area.y & 0xFF) as u8, ((area.y >> 8) & 0xFF) as u8,
            (area.width & 0xFF) as u8, ((area.width >> 8) & 0xFF) as u8,
            (area.height & 0xFF) as u8, ((area.height >> 8) & 0xFF) as u8,
        ]
    }
}
...
mod tests {
    ...
    fn set_print_area_encodes() {
        let area = PrintArea { x: 0, y: 0, width: 512, height: 400 };
...
```bash
cargo test -p bixolon command::page_mode
git add src/command/page_mode.rs src/command/mod.rs
git commit -m "feat: add page mode commands"
```
