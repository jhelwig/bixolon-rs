...
|---------|---------|-------------|
| `StartMacroDefinition` | `GS :` | Start macro definition |
| `EndMacroDefinition` | `GS :` | End macro definition (same command) |
| `ExecuteMacro` | `GS ^ r t m` | Execute defined macro |

---

## Implementation Notes

- Macro definition uses the same command to start and end (toggle behavior)
- Macro can store up to 2048 bytes
- Execution can repeat and specify wait time
...
mod tests {
    ...
    fn execute_macro_enforces_min_times() {
        let cmd = ExecuteMacro { times: 0, wait_100ms: 0, mode: MacroExecutionMode::Continuous };
...
```bash
git add src/command/macro_cmd.rs src/command/mod.rs
git commit -m "feat: add macro definition and execution commands"
```
