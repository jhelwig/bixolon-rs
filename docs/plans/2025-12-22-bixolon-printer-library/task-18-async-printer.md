# Task 18: Async Printer

> **Context:** See [index.md](./index.md) for architecture and rationale

**Files:**
- Create: `src/printer/async.rs`
- Modify: `src/printer/mod.rs` (add conditional export)

**Important:** Feature-gated behind `async` feature. Uses `tokio::io::{AsyncRead, AsyncWrite}`.
...
impl<W: AsyncWrite + Unpin, R> AsyncPrinter<W, R> {
    ...
    pub async fn send(&mut self, cmd: &impl Command) -> Result<(), PrinterError> {
    ...
    ///
    /// Does not flush - call `flush()` to ensure data is sent.
    pub async fn send_raw(&mut self, bytes: &[u8]) -> Result<(), PrinterError> {
    ...
    pub async fn print(&mut self, text: impl Into<StyledNode>) -> Result<(), PrinterError> {
        let node = text.into();
    ...
    pub async fn println(&mut self, text: impl Into<StyledNode>) -> Result<(), PrinterError> {
        let node = text.into();
    ...
    pub async fn print_page(&mut self, page: &PageBuilder) -> Result<(), PrinterError> {
        self.writer.write_all(&page.build()).await?;
        Ok(())
    }

    /// Print a page mode document and return to standard mode.
    pub async fn print_page_and_exit(&mut self, page: &PageBuilder) -> Result<(), PrinterError> {
        self.writer.write_all(&page.build_and_exit()).await?;
        Ok(())
    }

    /// Initialize the printer (reset to defaults).
    pub async fn initialize(&mut self) -> Result<(), PrinterError> {
    ...
    pub async fn flush(&mut self) -> Result<(), PrinterError> {
...
```bash
git add src/printer/async.rs src/printer/mod.rs Cargo.toml
git commit -m "feat: add async printer interface (tokio)"
```
