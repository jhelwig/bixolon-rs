...
impl StyleSet {
    ...
    pub fn from_stack(stack: &[StyleSet]) -> Self {
        ...
        for style in stack {
            ...
            if style.double_underline.is_some() {
                merged.double_underline = style.double_underline;
            }
    ...
    pub const fn with_double_underline(mut self, enabled: bool) -> Self {
        self.double_underline = Some(enabled);
        self
    }
...
pub fn style_transition_commands(from: &StyleSet, to: &StyleSet) -> Vec<Vec<u8>> {
    ...
    // Underline
    let from_underline = from.underline.unwrap_or(false);
    let to_underline = to.underline.unwrap_or(false);
    let from_double_underline = from.double_underline.unwrap_or(false);
    let to_double_underline = to.double_underline.unwrap_or(false);

    if from_underline != to_underline || from_double_underline != to_double_underline {
        let thickness = if to_double_underline {
            UnderlineThickness::TwoDot
        } else if to_underline {
            UnderlineThickness::OneDot
        } else {
...
mod tests {
    ...
    fn from_stack_inherit() {
        ...
        assert_eq!(result.underline, Some(true)); // from inner
    }

    #[test]
    fn from_stack_nested_three_levels() {
        let level1 = StyleSet::default().with_bold(true).with_underline(true);
        ...
        assert_eq!(result.underline, Some(true)); // inherited from level1
    ...
    fn style_transition_multiple_changes() {
        let from = StyleSet::default();
        let to = StyleSet::default().with_bold(true).with_underline(true).with_reverse(true);
        let commands = style_transition_commands(&from, &to);
